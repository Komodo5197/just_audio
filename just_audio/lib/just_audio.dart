        ...
        final platform = active && !_disposed
            ? await (_nativePlatform = _pluginPlatform.init(InitRequest(
                ...
                mappingCallback: (id) async {
                  assert(_audioSources.containsKey(id), 'Audio source with ID $id does not exist!');
                  assert(_audioSources[id]! is MappingAudioSource,
                      'Audio source with ID $id is not a MappingAudioSource!');
                  final source = _audioSources[id]! as MappingAudioSource;
                  var newSource = await source.createAudioSource(source.identifier);
                  newSource ??= SilenceAudioSource(duration: Duration.zero);
                  _audioSources[newSource._id] = newSource;
                  return newSource._toMessage();
                })))
...
abstract class AudioSource {
  ...
  static UriAudioSource uri(Uri uri, {Map<String, String>? headers, dynamic tag, URIResolver? resolver}) {
    ...
    if (hasExtension(uri, 'mpd')) {
      return DashAudioSource(uri, headers: headers, tag: tag, resolver: resolver);
    } else if (hasExtension(uri, 'm3u8')) {
      return HlsAudioSource(uri, headers: headers, tag: tag, resolver: resolver);
    } else {
      return ProgressiveAudioSource(uri, headers: headers, tag: tag, resolver: resolver);
    }
  }
...
abstract class UriAudioSource extends IndexedAudioSource {
  ...
  UriAudioSource(this.uri, {this.headers, dynamic tag, Duration? duration, this.resolver})
...
class SilenceAudioSource extends IndexedAudioSource {
  ...
  @override
  IndexedAudioSourceMessage _toMessage() => SilenceAudioSourceMessage(id: _id, duration: duration);
}
...
///
/// NOTE: This is only officially supported on Android.
